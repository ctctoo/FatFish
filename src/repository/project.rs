...
use crate::repository::{collection, link, new_id, tag};
...
fn assemble_project(conn: &Connection, row: &Row) -> AppResult<Project> {
    ...
    Ok(Project {
        tags: tag::list_for_project(conn, &id)?,
        collections: collection::list_for_project(conn, &id)?,
        links: link::list_for_project(conn, &id)?,
        git_info,
        id,
        name: row.get("name")?,
        path: row.get("path")?,
...
pub fn exists_with_path(conn: &Connection, path: &str) -> AppResult<bool> {
...
pub fn insert(conn: &Connection, input: &ProjectInput) -> AppResult<Project> {
    let id = new_id();
    ...
    get(conn, &id)?.ok_or_else(|| AppError::NotFound(format!("project {id}")))
}
...
pub fn delete(conn: &Connection, id: &str) -> AppResult<usize> {
    Ok(conn.execute("DELETE FROM projects WHERE id = ?1", params![id])?)
}
...
pub fn get(conn: &Connection, id: &str) -> AppResult<Option<Project>> {
    ...
    match rows.next()? {
        Some(row) => Ok(Some(assemble_project(conn, row)?)),
        None => Ok(None),
    }
}

pub fn set_favorite(conn: &Connection, id: &str, favorite: bool) -> AppResult<usize> {
    let n = conn.execute(
        "UPDATE projects SET favorite = ?2, updated_at = ?3 WHERE id = ?1",
        params![id, favorite as i64, crate::database::now()],
    )?;
    if n == 0 {
        return Err(AppError::NotFound(format!("project {id}")));
    }
...
pub fn mark_opened(conn: &Connection, id: &str) -> AppResult<usize> {
    let n = conn.execute(
        "UPDATE projects SET last_opened_at = ?2 WHERE id = ?1",
        params![id, crate::database::now()],
    )?;
    if n == 0 {
        return Err(AppError::NotFound(format!("project {id}")));
    }
...
pub fn list(conn: &Connection, filter: &ProjectFilter) -> AppResult<Vec<Project>> {
    ...
    let mut stmt = conn.prepare(&sql)?;
    let mut rows = stmt.query(params_from_iter(bind_values.iter().map(|b| b.as_ref())))?;
    // 逐行组装：assemble_project 需要再查关联表，因此不能在 query_map 闭包里做
    // （闭包只能返回 rusqlite::Result，无法承载 AppError）。
    let mut projects = Vec::new();
    while let Some(row) = rows.next()? {
        projects.push(assemble_project(conn, row)?);
    }
    Ok(projects)
}
