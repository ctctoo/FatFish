...
pub fn insert(conn: &Connection, input: &TodoInput) -> AppResult<Todo> {
    let id = new_id();
...
pub fn update(conn: &Connection, id: &str, input: &TodoInput) -> AppResult<usize> {
    let n = conn.execute(
...
pub fn set_done(conn: &Connection, id: &str, done: bool) -> AppResult<usize> {
    let n = conn.execute(
