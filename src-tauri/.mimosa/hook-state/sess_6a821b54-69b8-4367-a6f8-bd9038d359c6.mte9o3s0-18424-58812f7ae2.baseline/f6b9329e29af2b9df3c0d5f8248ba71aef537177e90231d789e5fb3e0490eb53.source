...
pub fn list(conn: &Connection) -> rusqlite::Result<Vec<Collection>> {
...
pub fn list_for_project(conn: &Connection, project_id: &str) -> rusqlite::Result<Vec<Collection>> {
    let mut stmt = conn.prepare(
        "SELECT c.id, c.name
         FROM collections c
...
pub fn insert(conn: &Connection, input: &CollectionInput) -> rusqlite::Result<Collection> {
    let id = new_id();
    conn.execute(
...
pub fn update(conn: &Connection, id: &str, input: &CollectionInput) -> rusqlite::Result<usize> {
    conn.execute(
