...
use crate::repository::{git_repository, link_repository, project_repository};
...
pub fn import_scanned(conn: &Connection, paths: &[String]) -> Result<Vec<ScannedProject>, String> {
    ...
    for path in paths {
        ...
        if create(conn, input).is_ok() {
            imported.push(ScannedProject {
                name: scanner_service::project_name_from_path(path),
                path: path.to_string(),
                language: scanner_service::detect_language(dir),
